...
let  getSongByMid=(singermid)=>{
  // let url=`https://c.y.qq.com/v8/fcg-bin/fcg_v8_singer_track_cp.fcg?g_tk=5381&inCharset=utf-8&outCharset=utf-8&notice=0&platform=yqq&format=jsonp&loginUin=0&hostUin=0&needNewCode=0&singermid=${singermid}&order=listen&begin=0&num=100&songstatus=1`
...
//获取巅峰榜单
let getTopRank=()=>{
  let url='https://c.y.qq.com/v8/fcg-bin/fcg_myqq_toplist.fcg?g_tk=5381&inCharset=utf-8&outCharset=utf-8&notice=0&platform=h5&format=jsonp&uin=0&needNewCode=1&jsonpCallback=__jp0'
  return new Promise((resolve,reject)=>{
    jsonp(url,{param:'jsonpCallback'},(err,data)=>{
      if(err){ reject(err)}
      else{ resolve(data)}
...
//获取榜单歌单
let getRankList=(topid)=>{
  let url=`https://c.y.qq.com/v8/fcg-bin/fcg_v8_toplist_cp.fcg?g_tk=5381&inCharset=utf-8&outCharset=utf-8&notice=0&platform=h5&format=jsonp&topid=${topid}&needNewCode=1&uin=0&tpl=3&page=detail&type=top'
...
//热门搜索
let getHotSearch=()=>{
  let url='https://c.y.qq.com/splcloud/fcgi-bin/gethotkey.fcg?g_tk=5381&inCharset=utf-8&outCharset=utf-8&notice=0&platform=h5&format=jsonp&uin=0&needNewCode=1&jsonpCallback=__jp0'
  return new Promise((resolve,reject)=>{
    jsonp(url,{param:'jsonpCallback'},(err,data)=>{
      if(err){ reject(err)}
      else{ resolve(data)}
